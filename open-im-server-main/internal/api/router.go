...
func newGinRouter(ctx context.Context, client discovery.SvcDiscoveryRegistry, cfg *Config) (*gin.Engine, error) {
	...
	// certificate
	// Initialize User IP Database (shared by auth and IP modules)
	ipDB, err := initUserIPDatabase(ctx, cfg)
	if err != nil {
		return nil, err
	}

	{
		a := NewAuthApiWithIPDB(pbAuth.NewAuthClient(authConn), ipDB)
	...
	// Wallet module - direct DB integration (no gRPC)
	{
		walletDB, err := initWalletDatabase(ctx, cfg)
		if err != nil {
			return nil, err
		}
		wa := NewWalletApi(walletDB)

		// User-side wallet routes
		walletGroup := r.Group("/wallet")
		walletGroup.POST("/create_account", wa.CreateAccount)
		walletGroup.POST("/get_balance", wa.GetBalance)
		walletGroup.POST("/recharge", wa.Recharge)
		walletGroup.POST("/withdraw", wa.Withdraw)
		walletGroup.POST("/transfer", wa.Transfer)
		walletGroup.POST("/get_transactions", wa.GetTransactions)

		// Admin-side wallet routes
		walletAdminGroup := r.Group("/wallet/admin")
		walletAdminGroup.POST("/get_overview", wa.AdminGetOverview)
		walletAdminGroup.POST("/get_account_list", wa.AdminGetAccountList)
		walletAdminGroup.POST("/adjust_balance", wa.AdminAdjustBalance)
		walletAdminGroup.POST("/freeze_account", wa.AdminFreezeAccount)
		walletAdminGroup.POST("/unfreeze_account", wa.AdminUnfreezeAccount)
		walletAdminGroup.POST("/freeze_amount", wa.AdminFreezeAmount)
		walletAdminGroup.POST("/unfreeze_amount", wa.AdminUnfreezeAmount)
		walletAdminGroup.POST("/get_audit_logs", wa.AdminGetAuditLogs)
		walletAdminGroup.POST("/get_freeze_records", wa.AdminGetFreezeRecords)
	}

	// User IP & Role module - direct DB integration
	{
		ipApi := NewUserIPApi(ipDB)

		ipGroup := r.Group("/user/ip")
		ipGroup.POST("/get_info", ipApi.GetUserIPInfo)
		ipGroup.POST("/get_logs", ipApi.GetUserIPLogs)
		ipGroup.POST("/search", ipApi.SearchByIP)
		ipGroup.POST("/record_login", ipApi.RecordLoginIP)

		roleGroup := r.Group("/user/role")
		roleGroup.POST("/set", ipApi.SetAppRole)
		roleGroup.POST("/get", ipApi.GetAppRole)
	}

	// Receptionist invite code & greeting module - direct DB + gRPC integration
	{
		recepDB, err := initReceptionistDatabase(ctx, cfg, msg.NewMsgClient(msgConn), relation.NewFriendClient(friendConn))
		if err != nil {
			return nil, err
		}
		ra := NewReceptionistApi(recepDB)

		// 邀请码管理 (admin)
		codeGroup := r.Group("/receptionist/invite_code")
		codeGroup.POST("/gen", ra.GenInviteCode)
		codeGroup.POST("/get", ra.GetInviteCode)
		codeGroup.POST("/update_status", ra.UpdateInviteCodeStatus)
		codeGroup.POST("/delete", ra.DeleteInviteCode)
		codeGroup.POST("/search", ra.SearchInviteCodes)

		// 客户绑定
		bindGroup := r.Group("/receptionist/binding")
		bindGroup.POST("/bind", ra.BindCustomer)
		bindGroup.POST("/get", ra.GetBinding)
		bindGroup.POST("/page", ra.PageBindings)
		bindGroup.POST("/stats", ra.GetBindingStats)

		// 问候语管理
		greetGroup := r.Group("/receptionist/greeting")
		greetGroup.POST("/set", ra.SetGreeting)
		greetGroup.POST("/get", ra.GetGreeting)
		greetGroup.POST("/search", ra.SearchGreetings)

		// 注册时自动处理
		r.POST("/receptionist/on_register", ra.OnCustomerRegister)

		// 发送日志
		r.POST("/receptionist/send_logs", ra.SearchSendLogs)
	}
...
// initWalletDatabase creates the wallet database controller with MongoDB collections.
func initWalletDatabase(ctx context.Context, cfg *Config) (controller.WalletDatabase, error) {
	dbb := dbbuild.NewBuilder(&cfg.Mongo, &cfg.Redis)
	...
	accountMgo, err := mgo.NewWalletAccountMongo(db)
	if err != nil {
		return nil, err
	}
	txMgo, err := mgo.NewWalletTransactionMongo(db)
	if err != nil {
		return nil, err
	}
	freezeMgo, err := mgo.NewWalletFreezeRecordMongo(db)
	if err != nil {
		return nil, err
	}
	auditMgo, err := mgo.NewWalletAuditLogMongo(db)
	if err != nil {
		return nil, err
	}

	return controller.NewWalletDatabase(accountMgo, txMgo, freezeMgo, auditMgo, mgocli.GetTx()), nil
}

// initUserIPDatabase creates the user IP database controller with MongoDB collections.
func initUserIPDatabase(ctx context.Context, cfg *Config) (controller.UserIPDatabase, error) {
	dbb := dbbuild.NewBuilder(&cfg.Mongo, &cfg.Redis)
	mgocli, err := dbb.Mongo(ctx)
	if err != nil {
		return nil, err
	...
	ipLogMgo, err := mgo.NewUserIPLogMongo(db)
	if err != nil {
		return nil, err
	}
	userMgo, err := mgo.NewUserMongo(db)
	if err != nil {
		return nil, err
	}

	return controller.NewUserIPDatabase(ipLogMgo, userMgo), nil
}

// initReceptionistDatabase creates the receptionist database controller with MongoDB collections.
func initReceptionistDatabase(ctx context.Context, cfg *Config, msgClient msg.MsgClient, friendCli relation.FriendClient) (controller.ReceptionistDatabase, error) {
	dbb := dbbuild.NewBuilder(&cfg.Mongo, &cfg.Redis)
	mgocli, err := dbb.Mongo(ctx)
	if err != nil {
		return nil, err
	...
	inviteCodeMgo, err := mgo.NewReceptionistInviteCodeMongo(db)
	if err != nil {
		return nil, err
	}
	bindingMgo, err := mgo.NewCustomerReceptionistBindingMongo(db)
	if err != nil {
		return nil, err
	}
	greetingMgo, err := mgo.NewReceptionistGreetingMongo(db)
	if err != nil {
		return nil, err
	}
	sendLogMgo, err := mgo.NewGreetingSendLogMongo(db)
	if err != nil {
		return nil, err
	}

	return controller.NewReceptionistDatabase(inviteCodeMgo, bindingMgo, greetingMgo, sendLogMgo, msgClient, friendCli), nil
}
